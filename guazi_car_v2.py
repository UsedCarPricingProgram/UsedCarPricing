...
def get_car_details_by_clueid(clueId) -> ():
    ...
    while the_number_of_failure < MAX_RETRY:
        try:
            session = requests.session()
            response = session.get(url=f'https://mapi.guazi.com/car-source/carDetail/detail?{query_string}',headers=headers)
            res = response.text
...
if __name__ == '__main__':
    ...
    details_writer = xlwt.Workbook()
    clue_id_data_writer = xlwt.Workbook()
    details_worksheet = details_writer.add_sheet("all_details")
    clue_id_data_worksheet = clue_id_data_writer.add_sheet("clue_id")
    ...
    for each_clue_id, index in zip(clue_id_list, range(len(clue_id_list))):
        print(f"{time.time()} 正在读取clue_id: {each_clue_id[0]}")
        if int(each_clue_id[1]) != 1:  # 不为1就需要去爬
            ...
            if len(res) != 0:  # 说明有结果, 成功
                ...
                clue_id_data_worksheet.write(index, 1, 1)
                for y in range(len(res)):
                    details_worksheet.write(index + 1, y, res[y])
                success_counter += 1
            else:
                clue_id_data_worksheet.write(index, 0, str(each_clue_id[0]))
                clue_id_data_worksheet.write(index, 1, 0)
        else:
            print(f"{time.time()} 当前club_id {each_clue_id[0]} 无需获取详细信息, 已跳过")

    details_writer.save(f"{int(time.time())}_TOTAL_RESULT_{success_counter}.xls")
    clue_id_data_writer.save(file_name)
